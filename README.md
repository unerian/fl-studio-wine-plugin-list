...
|     ✅    |Medium Testing    |nakst Apricot                    |1.1.34 x64| Has a native FL plugin version, which works as stable as stock FL plugins. 64bit CLAP seems to also work well. Sometimes if you move the plugin window from the monitor where it spawned to another monitor, then some knobs will snap to the max value when moved.  |
|     ✅    |Low Testing    |Auburn Sounds Graillon 3            |   3.2.0     |    Has a native FL plugin version, which works perfectly. VST3 seems to also work well. Since antares autotune doesn't work well, this can be a decent alternative|
...
|     ✅    |Low Testing    |Splice Bridge                    |5.1.1| Very simple plugin, seems to work perfectly. [Use this script to install splice itself.](https://github.com/MartinPrograms/splicer-installer-linux) If splice gives you an error, try running with "launch with terminal" enabled |
